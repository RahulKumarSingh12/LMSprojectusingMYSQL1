...
def add_record():
    ...
    if surety:
        try:
            cursor.execute(
                'INSERT INTO Library (BK_NAME, AUTHOR_NAME, BK_STATUS, CARD_ID) VALUES (%s, %s, %s, %s)',
                (bk_name.get(), author_name.get(), bk_status.get(), card_id.get())
            )
            connector.commit()
            clear_and_display()
...
def change_availability():
    global card_id, tree, cursor, connector
